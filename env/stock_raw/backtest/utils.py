...
class BacktestStats(JsonFile):
    ...
    def make(self):
        ...
        days_traded = df.loc[is_traded_day, 'day_pnl'].count() #计算非空值的数量
        ...
        stats['win_rate'] = days_win / float(days_traded) if has_traded else 0  #计算胜率
        pnl_total_sum = df.loc[:, 'day_pnl'].sum() 
        stats['day_traded_pnl_mean'] = pnl_total_sum / \
                                       days_traded if has_traded else 0
        pnl_std = df.loc[:, 'day_pnl'].std(ddof=0)#ddof=0表示无偏估计,自由度为n
